...
- **Why use a HashMap?** A `HashMap` lets you store and retrieve information instantly. We can use it to remember a number we've seen and, more importantly, _where_ we saw it (its index).
...
- **Steps:**
  ...
  6.  **No pair yet?** If the complement is not in the map, add the _current number_ and its index to the map. This is so you can use it for a potential match later in the loop.
...
- **Decode (Single String to List):**
  ...
  3.  **Loop** as long as your position `i` is inside the string:
      ...
      e. **Crucially**, update your position `i` to be at the start of the _next_ encoded block (right after the word you just extracted).
  4.  Return the list of words.

### Question 7: Product of Array Except Self

**Goal:** Given a list of numbers, create a new list where each element at index `i` is the product of all numbers in the original list _except_ for the one at index `i`.
...
- **Steps:**
  ...
  2.  **The Prefix Pass (Left-to-Right):**
      ...
      - For each position `i`, we first set `result[i]` to the current `prefix_product`. This stores the product of all the numbers that came _before_ it.
  ...
  3.  **The Postfix Pass (Right-to-Left):**
      ...
      - For each position `i`, we multiply the _existing_ value in `result[i]` (which currently holds the prefix product) by the `postfix_product`. This gives us the final answer for that position.
...
- **Steps:**
  ...
  3.  **For each cell:**
      ...
      _ Check the set for the current **row**. Have we seen this number in this row before? If yes, the board is invalid. Return `false`.
      _ Check the set for the current **column**. Have we seen this number in this column before? If yes, the board is invalid. Return `false`.
      _ Check the set for the current **3x3 box**. Have we seen this number in this box before? If yes, the board is invalid. Return `false`. (A simple formula `(row / 3) _ 3 + (col / 3)` can tell you which of the 9 boxes you are in).
...
**Goal:** Given a list of unsorted numbers, find the length of the longest possible sequence of consecutive numbers. For example, in `[100, 4, 200, 1, 3, 2]`, the longest consecutive sequence is `[1, 2, 3, 4]`, and its length is 4.

**Analogy:** Imagine you have a random pile of numbered puzzle pieces. You want to find the longest chain of pieces that fit together in order (like 5, 6, 7, 8).

**Method: Using a Hash Set for Smart Searching**

This method is efficient because it avoids sorting the entire list and instead cleverly identifies the _start_ of a potential sequence.

- **Why use a Hash Set?** A `HashSet` gives us a super-fast way to check if a number exists in our list, which is the main thing we need to do.

- **Steps:**
  1.  **Store all numbers for quick lookup:** First, add every number from the input list into a `HashSet`. This gets rid of duplicates and makes checking for numbers instantaneous.
  2.  **Loop through the unique numbers:** Go through each number in your `HashSet`.
  3.  **Identify the start of a sequence:** For each number, ask a crucial question: "Does the set also contain the number right _before_ this one (`num - 1`)?"
      - If the answer is "Yes," it means this number is _not_ the start of a sequence (e.g., if we are at `4` and `3` exists, we know `3` is the real start). So, we do nothing and move to the next number.
      - If the answer is "No," we have found the starting point of a potential sequence!
  4.  **Count the sequence length:**
      - Once you find a starting number (like `1` in the example), you can start counting how long this sequence is.
      - Keep checking for the next number in the sequence (`current_num + 1`) in the set.
      - As long as you find the next consecutive number, keep incrementing your count and checking for the next one.
  5.  **Keep track of the best score:** After you've counted a full sequence, compare its length to the longest one you've found so far and update your record if this one was longer.
  6.  **Return the record:** After checking all the numbers in the set, your record will hold the length of the longest consecutive sequence.
